...
- **MLEvolve 进化 / MLEvolve Evolution**: 记忆路由 (冷启动 KB + RRF 检索) + 锦标赛候选选择 (Elo + 变异算子) + 策略/战术解耦执行 (escalation) + 渐进式搜索调度 (熵衰减 + 停滞检测)
...
| 进化 Evolution | `evolution` | MLEvolve 集成: 记忆路由 (Phase 1) + 锦标赛选择 (Phase 2) + 解耦执行 (Phase 3) + 搜索调度器 (Phase 4) |
