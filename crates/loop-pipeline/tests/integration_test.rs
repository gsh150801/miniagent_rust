...
/// Helper: try to load API key, return None if not available.
/// Also sets DEEPSEEK_BASE_URL and other env vars so providers work.
fn try_load_api_key() -> Option<String> {
    // First, load all env vars from .env
    load_env_vars();

    // Now check env var (may have been set by load_env_vars())
    let key = std::env::var("DEEPSEEK_API_KEY").ok()?;
    if key.is_empty() || key.contains("your-key") || key.contains("sk-your") || key == "test-dummy-key-0000" {
        return None;
    }
    Some(key)
}
...
/// Test 7 (Online, requires API key): Verify planner actually calls the model
/// and returns a valid decomposition. Skipped if no API key.
#[tokio::test]
async fn test_planner_real_api_call() {
    ...
    let task = "Research the following three topics:\n\
                1. Latest advances in transformer neural networks\n\
                2. Quantum computing breakthroughs in 2024\n\
                3. CRISPR gene editing clinical trials";

    let prompt = format!(
        r#"You are the **Planner** in a multi-agent pipeline. Your job is to decompose tasks into MULTIPLE independent sub-tasks.

## Task
{task}

## Available Agent Roles
- "researcher": Has web_search, web_fetch, pubmed_search tools.
- "executor": Has bash, read, write, edit, glob, grep tools.
- "writer": Has read, write, edit tools.
- "critic": Has read, web_search, web_fetch tools.
- "synthesizer": Has read tool.
- "analyst": Has read, grep, glob tools.

## Instructions
1. Break the task into 3-8 concrete sub-tasks
2. For multi-topic queries, create one sub-task per topic
3. Assign "writer" or "synthesizer" for final compilation
4. Set depends_on correctly for dependent tasks
...
{{
  ...
  "max_loops": 5
    ...
    let provider = DeepSeekFlash::new(&key);
    let request = CompletionRequest {
        system: "You are an expert task planner. Output ONLY valid JSON.".into(),
    ...
    let response = match provider.complete(&request, cancel).await {
        Ok(r) => r,
        Err(e) => {
            eprintln!("⚠ SKIP: Planner API call failed (network/auth): {e}");
            return;
        }
    };
    ...
    for (i, t) in plan.tasks.iter().enumerate() {
        eprintln!("  {}. [{}] {} (deps: {:?})", i + 1, t.assigned_role, t.description, t.depends_on);
    }

    assert!(plan.tasks.len() >= 3,
        "Should have >= 3 tasks for 3 topics, got {}", plan.tasks.len());

    let researcher_count = plan.tasks.iter().filter(|t| t.assigned_role == "researcher").count();
    assert!(researcher_count >= 2, "Should have >= 2 researcher tasks");

    let has_writer = plan.tasks.iter().any(|t| t.assigned_role == "writer" || t.assigned_role == "synthesizer");
    assert!(has_writer, "Should have a writer/synthesizer task");

    eprintln!("\n✅ Planner produced valid decomposition with {} tasks", plan.tasks.len());
}

/// Test 8 (Online, requires API key): Verify Explorer actually calls tools.
/// Skipped if no API key.
#[tokio::test]
async fn test_explorer_real_api_call_with_tools() {
    let api_key = match try_load_api_key() {
        Some(k) => k,
        None => {
            eprintln!("⚠ SKIP: No DEEPSEEK_API_KEY available");
    ...
    let task = "What are the latest breakthroughs in fusion energy research?";

    let system_prompt = prompts::role_system_prompt("explorer", task, "Research findings");

    let instructions = format!(
        r#"You are the **Explorer** in a multi-agent pipeline.

## Original Task
{task}

## Instructions
1. Use **web_search** to research this topic.
2. Gather real information.

## Output Format (valid JSON only) at the end:
{{
  "clarified_task": "...",
  ...
  "needs_decomposition": true|false
    ...
    let user_prompt = format!("{}\n\n{}", instructions, prompts::tool_instruction_block());
    ...
    let agent = Agent::new(flash, pro).with_tools(executor);
    ...
    context.max_tool_iterations = 10;
    context.max_tokens = Some(4000);

    let cancel = tokio_util::sync::CancellationToken::new();
    let result = agent.run_with_loop(&mut history, &context, cancel).await;
    let _delta = match result {
        Ok(d) => d,
        Err(e) => {
            eprintln!("⚠ SKIP: Explorer agent run failed (network/auth): {e}");
            return;
        }
    };

    // Count tool usage
    let tool_call_count = history.iter()
        .filter(|m| m.role == miniagent_core::message::MessageRole::Tool)
        .count();

    let tool_use_count = history.iter()
        .filter(|m| m.role == miniagent_core::message::MessageRole::Assistant)
        .flat_map(|m| &m.content)
        .filter(|b| matches!(b, miniagent_core::event::ContentBlock::ToolUse { .. }))
        .count();

    eprintln!("\n=== Explorer Real API Result ===");
    eprintln!("Tool results in history: {}", tool_call_count);
    eprintln!("Tool use requests: {}", tool_use_count);
    eprintln!("Messages in history: {}", history.len());

    assert!(tool_use_count > 0 || tool_call_count > 0,
        "Explorer should make at least one tool call");

    eprintln!("\n✅ Explorer made {} tool calls and returned results", tool_use_count);
}
...
async fn test_full_pipeline_multi_loop() {
    ...
    eprintln!("\n═══ Running Full Loop Pipeline (3 loops) ═══");
    eprintln!("  This tests: multi-iteration exploration, planning, execution, evaluation, and repair\n");

    let config = Arc::new(AppConfig::load());
    let result = miniagent_loop_pipeline::LoopPipeline::run(
        "Research the latest developments in two areas:\n\
         1. Fusion energy breakthroughs (ITER, Commonwealth Fusion, etc.)\n\
         2. Quantum computing advances (error correction, logical qubits)\n\
         Then synthesize both into a report. Use web_search to gather real information.",
        config,
        3,  // 3 loops
        cancel,
        None,
    ...
    match result {
        Ok(output) => {
            ...
            eprintln!("{}", preview);
            assert!(!output.is_empty(), "Pipeline should produce non-empty output after 3 loops");
            eprintln!("\n✅ 3-loop pipeline test passed with {} output chars", output.len());
        }
        Err(e) => {
            eprintln!("WARN: Pipeline error (network/API): {e}");
        }
    }
}
...
async fn test_full_loop_pipeline_real_api() {
    ...
    eprintln!("\n═══ Running Full Loop Pipeline (1 loop) ═══");

    let config = Arc::new(AppConfig::load());
    let result = miniagent_loop_pipeline::LoopPipeline::run(
        "Research solid-state batteries for EVs: key breakthroughs and timeline.",
        config,
        1,
        cancel,
        None,
...
/// Test (Online): Full LoopPipeline run with 20 max loops, 30M token budget, 30 min timeout.
/// Real task: summarize new pathogenic mechanisms and novel targets in autoimmune diseases,
...
async fn test_real_long_running_research_pipeline() {
    ...
    unsafe {
        std::env::set_var("TOKEN_BUDGET", "30000000");
        std::env::set_var("LOOP_MAX_LOOPS", "20");
    }

    let cancel = tokio_util::sync::CancellationToken::new();

    // 30 minute timeout via tokio::time::timeout
    let timeout_duration = std::time::Duration::from_secs(30 * 60);

    eprintln!("\n═══════════════════════════════════════════════════════════════");
    eprintln!("  🧪 Real Long-Running Research Pipeline");
    eprintln!("  Task: New mechanisms & targets in autoimmune/neurodegenerative/oncology");
    eprintln!("  Max loops: 20 | Token budget: 30M | Timeout: 30min");
    eprintln!("═══════════════════════════════════════════════════════════════\n");

    let task = "Summarize the latest 5 years of research on:\n\
                 1. Autoimmune diseases: new pathogenic mechanisms and novel therapeutic targets\n\
                 2. Neurodegenerative diseases: new pathogenic mechanisms and novel therapeutic targets\n\
                 3. Oncology: new pathogenic mechanisms and novel therapeutic targets\n\
                 For each disease area, identify:\n\
                 - Key molecular pathways discovered in the last 5 years\n\
                 - Novel drug targets that have emerged\n\
                 - Promising therapeutic approaches in clinical trials\n\
                 - Cross-disease mechanisms (pathways shared across disease areas)\n\
                 Use web_search and pubmed_search to gather real, current information.\n\
                 Produce a comprehensive report with cited sources.";

    let config = Arc::new(AppConfig::load());
    let result = tokio::time::timeout(
        timeout_duration,
        miniagent_loop_pipeline::LoopPipeline::run(
            task,
            config,
            20,  // max 20 loops
            cancel,
    ...
    match result {
        Ok(Ok(output)) => {
            ...
            eprintln!("{}", preview);
            assert!(!output.is_empty(), "Pipeline should produce non-empty output");
            eprintln!("\n✅ Long-running research pipeline test PASSED with {} output chars", output.len());

            // Quality checks on output
            let lower = output.to_lowercase();
            let has_autoimmune = lower.contains("autoimmune") || lower.contains("lupus") || lower.contains("arthritis");
            let has_neuro = lower.contains("neurodegenerative") || lower.contains("alzheimer") || lower.contains("parkinson");
            let has_oncology = lower.contains("oncology") || lower.contains("cancer") || lower.contains("tumor");
            let has_mechanisms = lower.contains("mechanism") || lower.contains("pathway") || lower.contains("signaling");
            let has_targets = lower.contains("target") || lower.contains("therapeutic") || lower.contains("drug");

            eprintln!("\n  Content verification:");
            eprintln!("    Autoimmune area: {}", if has_autoimmune { "✅" } else { "❌" });
            eprintln!("    Neurodegenerative area: {}", if has_neuro { "✅" } else { "❌" });
            eprintln!("    Oncology area: {}", if has_oncology { "✅" } else { "❌" });
            eprintln!("    Mechanisms/pathways: {}", if has_mechanisms { "✅" } else { "❌" });
            eprintln!("    Targets/therapeutics: {}", if has_targets { "✅" } else { "❌" });

            assert!(has_autoimmune || has_neuro || has_oncology,
                "Output should cover at least one disease area");
            eprintln!("\n✅ Research pipeline quality checks passed");
        }
        Ok(Err(e)) => {
            eprintln!("WARN: Pipeline error (network/API): {e}");
            eprintln!("  This is expected if API is unavailable or times out.");
        }
        Err(_) => {
            eprintln!("WARN: Pipeline timed out after 30 minutes");
            eprintln!("  This is expected for long-running research tasks.");
        }
    }
