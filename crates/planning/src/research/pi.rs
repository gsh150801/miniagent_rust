...
impl AgentRole for PrincipalInvestigatorRole {
    ...
    async fn execute(
        ...
        let arena_json = blackboard.artifacts.get("tournament_arena")
            .cloned()
            .unwrap_or_default();
        let evidence_json = blackboard.get("evidence_accumulator/evidence.json")
            .unwrap_or_default();
        let observer_context = blackboard.get("observer/context.json")
            .unwrap_or_default();
...
"#
        ...
        blackboard.put("pi/decision.json", &content)?;
