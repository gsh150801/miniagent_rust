...
impl AgentRole for SynthesisJudgeRole {
    ...
    async fn execute(
        ...
        let standings = blackboard.get("tournament_master/final_standings.json")
            .or_else(|| blackboard.get("tournament_master/round_report.json"))
            .unwrap_or_default();
        let evidence = blackboard.get("evidence_accumulator/evidence.json")
            .unwrap_or_default();
        let pi_decision = blackboard.get("pi/decision.json")
            .unwrap_or_default();
...
"#
        ...
        blackboard.put("synthesis_judge/synthesis.json", &content)?;

        // Also write a human-readable markdown report
        let md = Self::json_to_markdown(&parsed);
        blackboard.put("synthesis_judge/report.md", &md)?;
