...
use crate::roles::{
    ...
    load_checkpoint, append_event, parse_llm_json,
};
...
impl AgentRole for TournamentMasterRole {
    ...
    async fn execute(
        ...
        if arena.is_finished() {
            ...
            blackboard.put("tournament_master/final_standings.json", &summary)?;
        ...
        // Persist arena state: 内存快照（供 pi/scheduler 即时读取）+ 落盘（可观察性）
        ...
        blackboard.put("tournament_master/arena.json", &arena_json)?;
        ...
        blackboard.put("tournament_master/round_report.json", &content)?;
