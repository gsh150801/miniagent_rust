...
impl AgentRole for ObserverRole {
    ...
    async fn execute(
        ...
        // Collect all current state from blackboard
        let event_log = blackboard.get("events.log")
            .unwrap_or_default();
...
"#,
        ...
        blackboard.put("observer/context.json", &json)?;
        blackboard.put("observer/context.md", &parsed.content)?;
