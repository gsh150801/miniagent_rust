...
impl AgentRole for ProposerRole {
    ...
    async fn execute(
        ...
        let opponent_critique = blackboard.get("opponent/critique.json");
        let researcher_findings = blackboard.get("researcher/findings.json");
...
"#
        ...
        blackboard.put("proposer/hypothesis.json", &output_json)?;

        // 第二轮标记：若本轮有 opponent critique（反驳轮），写 rebuttal.json 标记文件。
        // control_shell 的 "proposer_revise_after_judge" 规则据此停止重复激活
        // （FileExistsAndNot 不再满足），防止无限循环。
        if opponent_critique.is_some() {
            blackboard.put("proposer/rebuttal.json",
                &serde_json::json!({
                    "round": "rebuttal",
                    "refined_hypothesis": parsed.content,
                    "timestamp": chrono::Utc::now().to_rfc3339(),
                }).to_string())?;
            append_event(&blackboard.work_dir, "proposer: rebuttal round completed");
        }
