...
fn control_shell_scientific_defaults() {
    ...
    assert_eq!(shell.rule_count(), 6); // 5 原始科学规则 + 1 proposer_revise_after_judge 反向触发
}
...
fn load_api_key() -> miniagent_core::secrets::ApiKey {
    // First try environment variable
    if let Ok(key) = std::env::var("DEEPSEEK_API_KEY") {
        return miniagent_core::secrets::ApiKey::new(key);
    }
        ...
        for line in std::io::BufRead::lines(std::io::BufReader::new(
            ...
            if let Some((key, value)) = line.trim().split_once('=') {
                if key == "DEEPSEEK_API_KEY" && !value.starts_with("sk-your") && !value.is_empty() {
                    return miniagent_core::secrets::ApiKey::new(value.to_string());
                }
            }
        }
    }
...
async fn e2e_ad_pipeline_with_real_llm() {
    ...
    graph = graph.add_agent(
        ...
        miniagent_planning::ModelTier::Flash,
    );
    graph = graph.add_agent(
        "seed_amyloid",
        ...
        miniagent_planning::ModelTier::Pro,
    );
    graph = graph.add_agent(
        "seed_tau",
