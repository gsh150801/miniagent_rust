...
impl Tool for EditTool {
    ...
    async fn execute(
        ...
        let path = crate::security::resolve_safe_path(path_str, std::path::Path::new(&ctx.working_dir))
            .map_err(|e| AgentError::tool("edit", e))?;

        let content = std::fs::read_to_string(&path)
            .map_err(|e| AgentError::tool("edit", format!("read '{}': {e}", path.display())))?;
        ...
        std::fs::write(&path, &new_content)
            .map_err(|e| AgentError::tool("edit", format!("write '{}': {e}", path.display())))?;

        Ok(ToolOutput {
            content: format!("Made {count} replacement(s) in {}", path.display()),
            metadata: None,
        })
    }
}
