...
impl Tool for WebSearchTool {
    ...
    async fn execute(
        ...
        // Read global health state to skip unhealthy backends
        let hstate = health::health_state().await;
        ...
        if bocha_key.is_some() {
            ...
            // All keys existed but all failed (or were skipped due to health)
            Err(AgentError::tool("web_search", "All search backends (Serper, Tavily, Bocha) failed or were unavailable"))
        } else {
            Ok(ToolOutput {
